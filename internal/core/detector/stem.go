...
func (d *Detector) Stem(word string) string {
	...
	if rxPrefixFirst.MatchString(word) {
		// Remove prefix
		rootFound, word = d.removePrefixes(word)
		if rootFound {
			return word
		}

		// Remove particle
		particle, word = d.removeParticle(word)
		if d.dict.Contains(word) {
			return word
		}

		// Remove possesive
		possesive, word = d.removePossesive(word)
		if d.dict.Contains(word) {
			return word
		}

		// Remove suffix
		suffix, word = d.removeSuffix(word)
		if d.dict.Contains(word) {
			return word
		}
	} else {
	...
	rootFound, word = d.loopPengembalianAkhiran(originalWord, removedSuffixes)
	if rootFound {
		return word
	}
...
func (d *Detector) removePrefixes(word string) (bool, string) {
	...
	for i := 0; i < 3; i++ {
		...
		removedPrefix, word, recodingChar = d.removePrefix(word)
		if d.dict.Contains(word) {
			return true, word
		}

		for _, char := range recodingChar {
			if d.dict.Contains(char + word) {
...
func (d *Detector) removePrefix(word string) (prefix string, result string, recoding []string) {
	...
	case "di", "ke", "se", "ku":
		result = word[2:]
	case "me":
		result, recoding = d.removePrefixMe(word)
	case "pe":
		result, recoding = d.removePrefixPe(word)
	case "be":
		result, recoding = d.removePrefixBe(word)
	case "te":
		result, recoding = d.removePrefixTe(word)
	default:
		result, recoding = d.removeInfix(word)
	}

	return prefix, result, recoding
}

func (d *Detector) removePrefixMe(word string) (string, []string) {
	// Pattern 1
...
func (d *Detector) removePrefixPe(word string) (string, []string) {
	// Pattern 1
	// pe{w|y}V => pe-{w|y}V
...
func (d *Detector) removePrefixBe(word string) (string, []string) {
	// Pattern 1
	// berV => ber-V OR be-rV
...
func (d *Detector) removePrefixTe(word string) (string, []string) {
	// Pattern 1
	// terV => ter-V OR te-rV
...
func (d *Detector) removeInfix(word string) (string, []string) {
	// Pattern 1
	// CerV => CerV OR CV
